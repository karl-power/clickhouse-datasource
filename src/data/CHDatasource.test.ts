...
describe('ClickHouseDatasource', () => {
    ...
    it('returns true for hint-matched column via log alias', () => {
      const queryWithLevel: CHBuilderQuery = {
        ...
        builderOptions: {
          ...query.builderOptions,
          columns: [{ name: 'SeverityText', hint: ColumnHint.LogLevel, type: 'string' }],
          filters: [{ condition: 'AND', key: '', hint: ColumnHint.LogLevel, type: 'string', filterType: 'custom', operator: FilterOperator.Equals, value: 'info' }],
        },
      };
    ...
    it('returns true for OTel map key match', () => {
      const queryWithMap: CHBuilderQuery = {
        ...
        builderOptions: {
          ...query.builderOptions,
          columns: [{ name: 'ResourceAttributes', type: 'Map(String, String)' }],
          filters: [{ condition: 'AND', key: 'ResourceAttributes', mapKey: 'service.name', type: 'Map(String, String)', filterType: 'custom', operator: FilterOperator.Equals, value: 'my-service' }],
        },
      };

      expect(datasource.queryHasFilter(queryWithMap, { key: 'ResourceAttributes.service.name', value: 'my-service' })).toBe(true);
    });
  ...
  describe('toggleQueryFilter', () => {
    ...
    it('returns query unchanged for SQL mode', () => {
      const sqlQuery: CHSqlQuery = { pluginVersion: '', refId: 'A', editorType: EditorType.SQL, rawSql: 'SELECT 1' };
      const result = datasource.toggleQueryFilter(sqlQuery, { type: 'FILTER_FOR', options: { key: 'level', value: 'info' } });
      expect(result).toBe(sqlQuery);
    });
    ...
    it('FILTER_OUT replaces existing Equals filter with NotEquals', () => {
      const queryWithFilter: CHBuilderQuery = {
        ...
        builderOptions: {
          ...query.builderOptions,
          filters: [{ condition: 'AND', key: 'level', type: 'string', filterType: 'custom', operator: FilterOperator.Equals, value: 'error' }],
        },
      };
